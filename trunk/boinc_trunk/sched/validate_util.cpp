...
int VALIDATOR_FILE_INFO::parse(XML_PARSER& xp) {
    char tag[256];
...
int get_output_file_info(RESULT& result, VALIDATOR_FILE_INFO& fi) {
...
int get_output_file_infos(RESULT& result, vector<VALIDATOR_FILE_INFO>& fis) {
    char tag[256], path[1024];
    bool is_tag;
    MIOFILE mf;
    string name;
...
int get_output_file_paths(RESULT& result, vector<string>& paths) {
    vector<VALIDATOR_FILE_INFO> fis;
...
const char *BOINC_RCSID_07049e8a0e = "$Id: validate_util.cpp 18831M 2012-01-02 16:24:52Z (lokal) $";
