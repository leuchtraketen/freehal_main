...
#include "validate_util.cpp"
#include "validate_util2.h"
#include "validate_util2.cpp"
#include "validator-inc.cpp"
#include "sample_trivial_validator.cpp"
...
//    signal(SIGTERM, SIG_IGN);
    ...
    //
    char WUs[256000];
    retval = validator.enumerate_2(
        items,
        &result_handler,
        WUs
    );
    log_messages.printf(MSG_DEBUG,
        "CGI-VAL-ASS-DEL-PUR: items.size(): %d\n", items.size()
    );
    ...
    if (items.size()) {
        ...
        log_messages.printf(MSG_DEBUG,
            "CGI-VAL-ASS-DEL-PUR: SQL (1): %s\n", _buf
        );
        while (1) {
            retval = wu.enumerate(_buf);
            if (retval)
                break;
            ...
            if (retval) {
                log_messages.printf(MSG_CRITICAL,
                    "[WU#%d] CGI-VAL-ASS-DEL-PUR: FAILED: file deletion: %d\n", wu.id, retval
                );
            } else {
                log_messages.printf(MSG_DEBUG,
                    "[WU#%d] CGI-VAL-ASS-DEL-PUR: file deleted: %d\n", wu.id, retval
                );
            }
            ...
            if (retval) {
                log_messages.printf(MSG_CRITICAL,
                    "[WU#%d] CGI-VAL-ASS-DEL-PUR: FAILED: results purge:%d\n", wu.id, retval
                );
            }
            log_messages.printf(MSG_DEBUG,
                "[WU#%d] CGI-VAL-ASS-DEL-PUR: Purged results from database\n", wu.id
            );

            retval = wu.delete_from_db();
            if (retval) {
                log_messages.printf(MSG_CRITICAL,
                    "[WU#%d] CGI-VAL-ASS-DEL-PUR: FAILED: workunit purge:%d\n", wu.id, retval
                );
            }
            log_messages.printf(MSG_DEBUG,
                "[WU#%d] CGI-VAL-ASS-DEL-PUR: Purged workunit from database\n", wu.id
            );
        }
    }
