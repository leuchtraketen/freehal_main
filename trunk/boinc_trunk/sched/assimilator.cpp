...
bool do_pass(APP& app) {
    ...
    if (retval) {
        log_messages.printf(MSG_CRITICAL,
            "[WU#%d] ASS-DEL-PUR: FAILED: file deletion: %d\n", wu.id, retval
        );
    } else {
        log_messages.printf(MSG_DEBUG,
            "[WU#%d] ASS-DEL-PUR: file deleted: %d\n", wu.id, retval
        );
    }
    ...
    if (retval) {
        log_messages.printf(MSG_CRITICAL,
            "[WU#%d] ASS-DEL-PUR: FAILED: results purge:%d\n", wu.id, retval
        );
        exit(6);
    }
    log_messages.printf(MSG_DEBUG,
        "[WU#%d] ASS-DEL-PUR: Purged results from database\n", wu.id
    );

    retval = wu.delete_from_db();
    if (retval) {
        log_messages.printf(MSG_CRITICAL,
            "[WU#%d] ASS-DEL-PUR: FAILED: workunit purge:%d\n", wu.id, retval
        );
        exit(6);
    }
    log_messages.printf(MSG_DEBUG,
        "[WU#%d] ASS-DEL-PUR: Purged workunit from database\n", wu.id
    );

/*

    sprintf(
        buf, "assimilate_state=%d, transition_time=%d, file_delete_state=1", 
...
const char *BOINC_RCSID_7841370789 = "$Id: assimilator.cpp 21181M 2012-01-02 16:24:51Z (lokal) $";
