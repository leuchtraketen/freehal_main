...
//
#define WUS_PER_ENUM        1500
#define RESULTS_PER_ENUM    5000

// how long to wait until delete antiques, and how often to do it
//
#define ANTIQUE_DELAY       3600
#define ANTIQUE_INTERVAL    60
#define ANTIQUE_LIMIT       500000
...
bool do_pass(bool retry_error) {
    ...
    sprintf(buf,
        "where ( ( file_delete_state=%d ) OR (file_delete_state=%d and validate_state <> %d ) ) %s limit %d",
        retry_error?FILE_DELETE_ERROR:FILE_DELETE_READY,

FILE_DELETE_INIT, VALIDATE_STATE_INIT,
    ...
    if (result.file_delete_state == FILE_DELETE_INIT) result.file_delete_state = FILE_DELETE_READY;
...
const char *BOINC_RCSID_bd0d4938a6 = "$Id: file_deleter.cpp 21181M 2012-01-02 16:24:51Z (lokal) $";
