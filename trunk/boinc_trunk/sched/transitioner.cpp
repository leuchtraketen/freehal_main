...
#define SELECT_LIMIT    800

#define DEFAULT_SLEEP_INTERVAL  0.5

using namespace std;

int workunits_over =0;
int startup_time;
...
int handle_wu(
    ...
    if (mod_n && wu_item.id % mod_n != mod_i) return 0;
    ...
    if (wu_item.error_mask) {
        ...
        if (n_new_results_needed > 0) {
            ...
            ofstream o("/tmp/new-results-recent");
            o << "1" << endl;
            o.close();
    ...
    //
    if (wu_item.assimilate_state == ASSIMILATE_DONE && (    0 ||    (most_recently_returned) <= now)) {
...
//            (int)time(0), SELECT_LIMIT, mod_n, mod_i, items
            (int)time(0), SELECT_LIMIT, 0, 0, items, workunits_over
        );
        if (retval) {
...
const char *BOINC_RCSID_be98c91511 = "$Id: transitioner.cpp 21230M 2012-01-02 16:24:52Z (lokal) $";
