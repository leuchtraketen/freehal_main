...
bool work_needed(bool locality_sched) {
    ...
    int need_workunits = 1;
    int need_one_per_core = strstr(mode_str(), "_onepercore_") ? 1 : 0;
    if (need_one_per_core) {
        need_workunits = g_reply->host.p_ncpus;

        if (strstr(mode_str(), "_noht_")) {
            need_workunits = need_workunits / 2;
        }

        if (need_workunits > 8) need_workunits = 8;
        if (need_workunits < 1) need_workunits = 1;
    }

        log_messages.printf(MSG_NORMAL, "NEEDED: %d workunits\n", need_workunits);

    static int claim_of_type_already_mentioned = 0;
    if (!claim_of_type_already_mentioned) {
        ...
        claim_of_type_already_mentioned = 1;
    }

if (nwus >= need_workunits) {

    log_messages.printf(MSG_NORMAL, "There are enough work units now: %d\n", nwus);
    return false;
}
static int we_have = 0;
if (nwus != we_have) {
    we_have = nwus;
    log_messages.printf(MSG_NORMAL, "We still need work units! (we have %d)\n", nwus);
}
return true;
...
const char *BOINC_RCSID_32dcd335e7 = "$Id: sched_send.cpp 21362M 2012-01-02 16:24:51Z (lokal) $";
