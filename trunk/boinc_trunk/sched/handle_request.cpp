...
static inline bool requesting_work() {
    ...
    if (g_request->coproc_cuda && g_request->coproc_cuda->req_secs) return false;
    if (g_request->coproc_ati && g_request->coproc_ati->req_secs) return false;
    return false;
}
...
void process_request(char* code_sign_key) {
    ...
    if (strstr(behave_str(), "_nci_")) {
        config.non_cpu_intensive = true;
        log_messages.printf(MSG_NORMAL,
            "config.non_cpu_intensive = true"
        );
    } else {
        config.non_cpu_intensive = false;
        log_messages.printf(MSG_NORMAL,
            "config.non_cpu_intensive = false"
        );
    }
    ...
    if (requesting_work()) {
                    log_messages.printf(MSG_NORMAL,
                        "ACTION: user is requesting work\n"
                    );
        ...
        if (g_wreq->no_jobs_available) {
            g_reply->insert_message("Project has no jobs available", "low");
                    ...
                    log_messages.printf(MSG_NORMAL,
                        "ACTION: user is NOT requesting work\n"
                    );
    }
...
const char *BOINC_RCSID_2ac231f9de = "$Id: handle_request.cpp 21181M 2012-01-02 16:24:51Z (lokal) $";
