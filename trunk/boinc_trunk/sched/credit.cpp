...
// dummy, um Probleme mit den Limits auszuschliessen
}
int grant_credit_fast(
    DB_HOST& host, double start_time, double cpu_time, double credit
) {
    DB_USER user;
    DB_TEAM team;
...
//    host.total_credit += credit;
    sprintf(
        buf, "total_credit=total_credit+%d, expavg_credit=%d, expavg_time=%d",
        (int)credit,  (int)host.expavg_credit, (int)host.expavg_time
    );
    ...
    sprintf(
        buf, "total_credit=total_credit+%d, expavg_credit=%d, expavg_time=%d",
        (int)credit,  (int)user.expavg_credit, (int)user.expavg_time
    );
    ...
    if (user.teamid) {
        ...
        sprintf(buf,
            "total_credit=total_credit+%d, expavg_credit=%d, expavg_time=%d",
            (int)credit,  (int)team.expavg_credit, (int)team.expavg_time
        );
