...
//printf("wi.wu.name: %s\n", wi.wu.name);
//        if (strstr(wi.wu.name, (___i%2 == 0)?"_nci":"_ci")) continue;
    ...
    for (i=0; i<ssp->max_wu_results; i++) {
        ...
        case WR_STATE_EMPTY:
            ...
            found = get_job_from_db(
                wi, app_index, enum_phase[app_index], ncollisions, i
            );
            if (found) {
                log_messages.printf(MSG_NORMAL,
...
int main(int argc, char** argv) {
    ...
    if (retval) {
        log_messages.printf(MSG_CRITICAL, "can't create shmem: %i + %i*%i = %i\n",
		sizeof(SCHED_SHMEM), num_work_items, sizeof(WU_RESULT), shmem_size);
        exit(1);
    }
    ssp = (SCHED_SHMEM*)p;
    ssp->init(num_work_items);
...
const char *BOINC_RCSID_57c87aa242 = "$Id: feeder.cpp 21181M 2012-06-05 15:07:25Z (lokal) $";
