...
int DB_BASE::insert() {
    ...
    db_print(vals);
    sprintf(query, "insert ignore into %s set %s", table_name, vals);
    return db->do_query(query);
}

int DB_BASE::insert_batch(std::string& values) {
    std::string query;
    query = "insert ignore into " + std::string(table_name) + " values " + values;
...
const char *BOINC_RCSID_43d919556b = "$Id: db_base.cpp 21109M 2010-06-18 18:49:13Z (lokal) $";
