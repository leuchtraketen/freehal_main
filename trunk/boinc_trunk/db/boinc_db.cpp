...
if (workunits_over) {
	sprintf(mod_clause,
//                " and assimilate_state = 2");
                " and assimilate_state != 0");
static int __i = 3;
if (__i > 2) {
        nresult_limit = 100;
	--__i;
...
int DB_VALIDATOR_ITEM_SET::enumerate(
    ...
    if (!cursor.active) {
        ...
        sprintf(query,
            ...
            "   and wu.appid = %d and (wu.need_validate <> 0 OR (res.outcome = 1 and res.client_state = 5 and res.server_state = 5 and res.validate_state IN (0, 1))) %s "
            "LIMIT "
            "   %d ",
...
int DB_VALIDATOR_ITEM_SET::enumerate_2(
    std::vector<VALIDATOR_ITEM>& items,
    DB_SCHED_RESULT_ITEM_SET* set,
    char* WUs
) {

    int nresult_limit = 100;
    ...
    if (!cursor.active) {
        ...
        char __buf[256];
        int i;
        for (i=0; i<set->results.size(); i++) {
            if (set->results[i].id == 0) continue;   // skip non-updated results


            if (set->results[i].outcome == RESULT_OUTCOME_INIT) continue;
            if (set->results[i].outcome == RESULT_OUTCOME_CLIENT_ERROR) continue;



            if (strstr(set->results[i].name, "asgn")) continue;  // skip assigned jobs
            sprintf(__buf, ", %d", set->results[i].workunitid);
            strcat(WUs, __buf);
        }
...
int DB_WORK_ITEM::enumerate(
    int limit, const char* select_clause, const char* order_clause, int ___i
) {
    char query[MAX_QUERY_LEN];
    int retval;
    MYSQL_ROW row;
...
int DB_SCHED_RESULT_ITEM_SET::update_workunits() {
    ...
    sprintf(query,
        "UPDATE workunit SET  need_validate=1 WHERE id in (",
        (int)time(0)
    );
    first = true;
    for (i=0; i<results.size(); i++) {
        ...
        if (results[i].outcome != RESULT_OUTCOME_SUCCESS) continue;
...
const char *BOINC_RCSID_ac374386c8 = "$Id: boinc_db.cpp 21415M 2010-11-09 17:52:44Z (lokal) $";
