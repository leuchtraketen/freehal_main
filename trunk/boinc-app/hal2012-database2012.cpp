...
database::database(dbimpl* impl) :
		freehal_base(), implementation(impl) {
}
database::~database() {
}

void database::set_lang(const string& _lang) {
	freehal_base::set_lang(_lang);
	resume();
}
void database::set_path(const fs::path& _path) {
	freehal_base::set_path(_path);
	resume();
}

void database::set_tagger(tagger* _t) {
	t = _t;
}
tagger* database::get_tagger() const {
	return t;
}

bool database::is_configured() const {
...
int database::prepare(const fs::path& p) {
	return prepare(p, p);
}

int database::prepare(const fs::path& p, const fs::path& as) {
	if (!is_configured())
		return 1;
		...
		if (fs::exists(p)) {
			...
			if (fs::is_regular_file(p)) {
				...
				if (files.find(as.generic_string()) == files.end()
					...
					dbimpl* idb = implementation->new_instance();
					read_xml_fact_file(p, idb);
					idb->to_disk(dbimpl::ALL_KEYS);
					delete idb;
...
const fs::path database::disk_find_file(const string type, T1 a, T1 b, T1 c,
		T1 d, T2 filename) {
...
template<typename M>
int database::disk_read_file(const fs::path& p, M& list) {
	if (!is_configured())
		return 1;
...
template<typename M>
int database::disk_write_file(const fs::path& p, const M& list) {
	if (!is_configured())
		return 1;
...
int database::find_by_word(vector<boost::shared_ptr<xml_fact> >& list,
		const word& word) {
	if (!is_configured())
		return 1;
	...
	std::for_each(list.begin(), list.end(),
			boost::bind(&database::insert_synonyms, this, _1));
	if (t != 0) {
...
int database::find_by_words(vector<boost::shared_ptr<xml_fact> >& list,
...
int database::find_by_fact(vector<boost::shared_ptr<xml_fact> >& list,
...
int database::read_xml_fact_file(const fs::path filename, dbimpl* idb) {
	if (!is_configured())
		return 1;
	{
		fs::ifstream i;
...
int database::get_synonyms(vector<word>& syns, const word& w) {
	if (!is_configured())
		return 1;
...
boost::shared_ptr<xml_obj> database::insert_synonyms(
...
int database::think() {
	...
	vector<boost::shared_ptr<consequence> > patterns;

	{
		vector<boost::shared_ptr<xml_fact> > facts;
		this->find_by_words(facts, consequence::get_if_words_list());

		foreach(boost::shared_ptr < xml_fact > xfact, facts)
		{
			cout << xfact->print_str() << endl;
			cout << xfact->print_text() << endl;
			cout << xfact->print_xml() << endl;

			boost::shared_ptr<consequence> pattern(new consequence(xfact));
			if (pattern->is_valid()) {
				patterns.push_back(pattern);
			}
		}

		foreach(boost::shared_ptr < consequence > pattern, patterns)
		{
			cout << pattern->print_str() << endl;
			cout << pattern->print_xml() << endl;
		}
	}

	{
		vector<boost::shared_ptr<xml_fact> > facts;
		this->find_by_word(facts, word("hoch"));

		boost::shared_ptr<consequence> pattern(patterns[patterns.size() - 1]);
		foreach(boost::shared_ptr < xml_fact > xfact, facts)
		{
			if (consequence::match(pattern, xfact)) {
				cout << endl;
				cout << xfact->print_str() << endl;
				cout << xfact->print_text() << endl;
				cout << xfact->print_xml() << endl;
