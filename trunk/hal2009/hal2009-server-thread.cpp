...
#include "hal2009.h"
#include "hal2009-server-thread.h"
...
void hal2009_server_statement(const string& s, const string& username, bool do_learn, bool do_talk) {
    ...
    while (output.size() == 0 && timeout > 0) {
        ...
        pthread_t answer_thread = hal2009_answer(input, current_instance->planguage, current_instance->tlanguage, current_instance->base_dir, NOT_JOIN, MULTI);
...
void hal2009_server_client_connection() {
    ...
    while (cin && getline(cin, line)) {
        ...
        if ( init_thread_ended > 0 && result->at(0) == string("QUESTION") && result->at(1) != string("QUESTION") && result->size() >= 2 && result->at(1).size() > 0 && !(result->at(1).size() < 3 && ' ' == result->at(1)[0]) ) {
            ...
            replace_all(input, "::", ":");
            hal2009_server_statement(input, username, true, true);
        }
        ...
        if ( init_thread_ended > 0 && result->size() >= 2 && result->at(1).size() > 0 && !(result->at(1).size() < 3 && ' ' == result->at(1)[0]) ) {
            ...
            if (result->at(0) == string("LEARN") && result->at(1) != string("LEARN")) {
                hal2009_server_statement(input, username, true, false);
            }
            else if (result->at(0) == string("TALK") && result->at(1) != string("TALK")) {
                hal2009_server_statement(input, username, false, true);
            }
...
void hal2009_server_run() {
    hal2009_server_client_connection();
}
...
void* hal2009_init_thread (void*) {
    ...
    hal2009_init(current_instance->planguage, current_instance->tlanguage, current_instance->base_dir);
    init_thread_ended = 1;
...
void* hal2009_handle_signal(void* _p) {
    ...
    if (type == "output_pos") {
        ...
        const string& pos = hal2009_server_get_value_from_socket("WORD_TYPE", text);
        hal2009_send_signal("input_pos", pos);
        unlink("_doing__pos");
    }
/*
...
*/
        ...
        struct DATASET set = hal2009_get_csv(text.c_str());
