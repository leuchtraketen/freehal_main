...
"     LANGUAGE      \n"
"  -e FILE          Execute FILE as HAL2012-Code by using the language from \"-p\"\n"
"  -c FILE          Compile FILE from HAL2012-Code to the language from \"-p\"\n"
"  -d FILE          Generae documentation for source file FILE (HAL2012-Code)\n"
"\n"
...
int main (int argc, char** argv) {
    ...
    if (compile_file.size() > 0) {
        hal2012_compile(compile_file, programming_language);
    }
    else if (execute_file.size() > 0) {
        extract();
        hal2012_execute_file(execute_file, programming_language);
    }
        ...
        pthread_t answer_thread = hal2012_answer(input, programming_language, language, base_dir, NOT_JOIN, SINGLE);
...
void shell(string programming_language, string language, string base_dir) {
    ...
    while (1) {
        ...
        pthread_t answer_thread = hal2012_answer(input, programming_language, language, base_dir, NOT_JOIN, MULTI);
        pthread_join(answer_thread, NULL);
        halsleep(1);
    }
}
