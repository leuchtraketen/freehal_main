...
int main (int argc, char** argv) {
    ...
    pthread_t signal_thread = hal2012_start_signal_handler(strdup(programming_language), strdup(language), MULTI);
    hal2012_init(strdup(programming_language), strdup(language), strdup(base_dir));
...
void hal2012_handle_signal(void* arg) {
	...
	set_user();
        ...
        pthread_t answer_thread = hal2012_answer(strdup(input), strdup(programming_language), strdup(language), strdup(base_dir), NOT_JOIN, MULTI);
    }
}
