...
const string define_general_verb(const string& sentence, const string& entity) {
    ...
    int position_ist = sentence.find(hal2012_get_text_language()=="de"?" ist ":" is ");
    int position_sind = sentence.find(hal2012_get_text_language()=="de"?" sind ":" are ");
    int position_war = sentence.find(hal2012_get_text_language()=="de"?" war ":" was ");
    int position_waren = sentence.find(hal2012_get_text_language()=="de"?" waren ":" were ");
    ...
    if (hal2012_get_text_language() == "de") {
        articles_indef.push_back("ein");
        articles_indef.push_back("eine");
        articles_indef.push_back("Ein");
        articles_indef.push_back("Eine");
    }
    if (hal2012_get_text_language() == "en") {
...
EXTERN_C struct fact** search_facts_wiki(const char* _entity, short todo) {
    ...
    /// if NEW
    if (todo == NEW) {
        string url = hal2012_get_text_language() + string(".wikipedia.org/w/index.php?title=Spezial%3ASuche&fulltext=Volltext&search=") + replace_spaces(entity_upper);
        cout << "url: " << url << endl;
...
struct fact** search_facts_wiki_page(const string& __url, const string& entity_upper) {
    ...
    string url = replace_spaces(hal2012_get_text_language() + string(".wikipedia.org") + __url);
    ...
    for (i = 0; i < lines.size(); ++i) {
        ...
        if (0 == in_header) {
            ...
            /// write to prot file
            {
                string file = "lang_" + string(hal2012_get_text_language()) + "/wiki/" + entity_upper[0] + "-wiki.prot";
