...
XS(XS_POSIX_constant)
{
    ...
    SP -= items;
    {
#line 2549 "const-xs.inc"
	STRLEN		len;
#line 1540 "POSIX.c"
	SV *	sv = ST(0);
	const char *	s = SvPV(sv, len);
#line 2554 "const-xs.inc"
#ifdef SYMBIAN
...
#endif
    if (items < 6 || items > 9)
       Perl_croak(aTHX_ "Usage: %s(%s)", "POSIX::asctime", "sec, min, hour, mday, mon, year, wday = 0, yday = 0, isdst = -1");
    PERL_UNUSED_VAR(cv); /* -W */
    {
...
#endif
    if (items < 6 || items > 9)
       Perl_croak(aTHX_ "Usage: %s(%s)", "POSIX::mktime", "sec, min, hour, mday, mon, year, wday = 0, yday = 0, isdst = -1");
    PERL_UNUSED_VAR(cv); /* -W */
    {
	int	sec = (int)SvIV(ST(0));
	int	min = (int)SvIV(ST(1));
...
#line 2 "const-xs.inc"
  {
    ...
    dTHX;
#endif
    HV *symbol_table = get_hv("POSIX::", TRUE);
#ifndef SYMBIAN
    HV *constant_missing;
#endif
